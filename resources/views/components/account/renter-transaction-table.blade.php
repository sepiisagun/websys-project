...
<tr class="border-b dark:border-gray-700">
	<th
		...
		scope="row"
	>{{ $reservation->first_name }} {{ $reservation->last_name }}</th>
	...
	{{-- <td class="px-4 py-3">₱{{ $house->first_name }}</td> --}}
	<td class="flex items-center justify-end px-4 py-3">
		<button
			...
			data-dropdown-toggle="house-{{ $reservation->id }}-dropdown"
			id="house-{{ $reservation->id }}-dropdown-button"
			type="button"
		>
			<svg
				aria-hidden="true"
		...
		<div
			...
			id="house-{{ $reservation->id }}-dropdown"
		>
			<ul
				aria-labelledby="house-{{ $reservation->id }}-dropdown-button"
				class="py-1 text-sm text-gray-700 dark:text-gray-200"
			>
				<li>
					<a
						class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
						href="{{ route('house.show', $reservation->id) }}"
					>Show</a>
				</li>
				<li>
					<a
						class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
						href="{{ route('house.edit', $reservation->id) }}"
					>Edit</a>
				</li>
			</ul>
			<div class="py-1">
				<a
					...
					data-modal-target="popup-modal-{{ $reservation->id }}"
					data-modal-toggle="popup-modal-{{ $reservation->id }}"
					type="button"
				>
					Delete
				</a>
